...
def show_winner1():
    ...
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Сгенерированное Число: ', number1, vremya[11:19]))
...
def show_winner2():
    ...
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Сгенерированное Большое Число: ', number2, vremya[11:19]))
...
def show_word():
    ...
    word.setText(neword)
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Сгенерированное Слово: ', neword, vremya[11:19]))
...
def show_phraser():
    ...
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Сгенерированная Фраза: ', newpraser, vremya[11:19]))


def show_just():
    msg.exec_()
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Неработающая Кнопка: ', 'Работает', vremya[11:19]))
        

def show_image():
    imagemsg = QMessageBox()
    which_im = randint(0, 10)
    kartinki = ['wow.png', 'НЕТ.png', 'лестница.png', 'Бар и бета.png', 'ОкноСКучей(Почти)БесполезныхКнопок,КотороеМеняетСвоиРазмерыПриКаждомОткрытии.jpg', 'Stwquia.png', 'Why Is Everyone So Mean To Me.png', 'куб2.png', '88888888ver2.png', 'XP.png', 'TV.png']
    imagemsg.setWindowTitle(kartinki[which_im])
    pixmap = QPixmap(kartinki[which_im])
    imagemsg.setIconPixmap(pixmap)    
    imagemsg.exec_()
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Название Картинки: ', kartinki[which_im], vremya[11:19]))
...
def show_count():
    ...
    #print(HowPressMany)
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Счётчик Кнопки Счётчика: ', HowPressMany, vremya[11:19]))


def show_press():
    dvig_x = randint(5,weight)
    dvig_y = randint(5,height)
    buttonPRESS.move(dvig_x, dvig_y)
    buttonPRESSold.hide()
    buttonPRESS.show()
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Положение Кнопки "Нажми На Меня!": ', 'x-' + str(dvig_x) + ' ' + 'y-' + str(dvig_y), vremya[11:19]))
...
def show_music():
    ...
    #print('sound')
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Проигрыш Звука: ', choose_of_sound[randmus], vremya[11:19]))    
...
def show_music2():
    ...
    #print('sound')
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Проигрыш Музыки: ', choose_of_music[whatmus], vremya[11:19])) 
...
def show_stop():
    ...
    buttonSOUND2.play2.play()
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Остановка Музыки: ', 'STOP', vremya[11:19])) 
...
def show_back():
    ...
    main_win.setPalette(pal)
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('Смена Цвета Фона (Активный\Неактивный): ', actbacklcolor + ' ' + inactbacklcolor, vremya[11:19])) 
...
def show_history():
    ...
    global historia_txt
    vremya = time.asctime()  
    cursor.execute('INSERT INTO Results (name, result, time) VALUES (?, ?, ?)', ('История: ', 'ОТКРЫТА ИСТОРИЯ', vremya[11:19]))
    cursor.execute('SELECT * FROM Results')
    results = cursor.fetchall()
